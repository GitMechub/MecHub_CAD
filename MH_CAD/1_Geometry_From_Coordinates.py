...
col1.subheader('Extrude Geometry',help='Points must be listed in sequential order, where each point connects to the previous one.'
               ,anchor=False)

upload_coordinates = col1.toggle("Upload Coordinates File", help='If you prefer to upload a file instead of setting the surface coordinates manually.'
                                          ' The points must be ordered.', key='st_upload_coordinates',on_change =desativ_extrude_button)
...
## Set Scale Input
set_scale = col1.toggle("Optional: Set Scale", help='This option allows you to adjust the scale of the drawing contour that has been set.'
                                          , value=False)
...
## Extruded Cut Input
extruded_cut = col1.toggle("Optional: Extruded Cut", help='If you want to perform an Extruded Cut, activate this option.'
...
if not extruded_cut:
    ...
    upload_coordinates_cut = col1.toggle("Upload Coordinates File",
                                     help='If you prefer to upload a file instead of setting the surface coordinates manually.'
                                          ' The points must be ordered.', value=False, on_change =desativ_extrude_button)

    if not upload_coordinates_cut:
        col1.write('Surface Coordinates (x, y)')
        df__coord_cut = pd.DataFrame({'x': st.session_state['st_coordinates_x_cut'], 'y': st.session_state['st_coordinates_y_cut']})
...
if preview_button:
    try:
        ...
        show_sketch_preview_2(df__coord_input, df__coord_input_cut, scale_) if extruded_cut else show_sketch_preview(df__coord_input, scale_)
    except Exception as e:
  ...
  with col2:
    ...
    with st.spinner('Loading...'):
        ...
        try:
            ...
            coordinates = [(round(float(df__coord_input['x'][i]), 6), round(float(df__coord_input['y'][i]), 6)) for i in range(len(df__coord_input['x']))]
            try:
                coordinates_cut = [(round(float(df__coord_input_cut['x'][i]), 6), round(float(df__coord_input_cut['y'][i]), 6)) for i in range(len(df__coord_input_cut['x']))]
            except:
                pass
            coordinates, extrusion_type, length, revolve_angle, revolve_axis = proc_EG_input(coordinates, extrusion_type,
            ...
            def extruded_cut_geometry(coordinates_cut, coordinates, solid, s_surface, length_cut,
                    ...
                    try:
                        ...
                        try:
                            ...
                            if coordinates_cut == coordinates and extruded_cut == True:
                                st.error("Error: The coordinates for the extrude cut are the same as those for the solid")
                                extruded_cut = False
                        ...
                        if set_scale is True and extrusion_type_cut == "basic":
                        ...
                        solid_cut = sketch.revolve(angleDegrees=revolve_angle_cut, axisStart=(0, 0, 0),
                                                   axisEnd=revolve_axis_cut) if extrusion_type_cut == "revolve" else sketch.extrude(
                            length_cut)
            ...
            if s_surface:
              ...
              try:
                ...
                if set_scale is True and extrusion_type == "basic":
                  length_ = length * scale
                else:
                  length_ = length

                ## Extrude or Revolve around the axis (Y is the default axis for revolve!)
                solid = sketch.revolve(angleDegrees=revolve_angle, axisStart=(0, 0, 0), axisEnd=revolve_axis) if extrusion_type == "revolve" else  sketch.extrude(length_)
                ...
                sketch_name, solid_name, solid = extruded_cut_geometry(coordinates_cut, coordinates_, solid, s_surface, length_cut, extrusion_type_cut, revolve_angle_cut, revolve_angle, revolve_axis_cut, extruded_cut, set_scale, scale)

              except Exception as e:
                st.error(f"Error creating extrusion: {e}. Try to alternate between 'basic' and 'revolve,' or verify the order of the coordinates.")
            ...
            try:
                ...
                if extruded_cut:
                    ...
                    df_coordinates_cut = scale_contour_df(df_coordinates_cut, scale) if set_scale else df_coordinates_cut
